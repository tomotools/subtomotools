...
@click.option('-r', '--radius',
              help='Radius of particle in pixels, for normalization.')
...
def project_particles(ctf, z_thickness, radius, input_star):
    ...
    projections = np.empty((len(particles.index), dim[1], dim[2]),
                           dtype=np.float32)
        ...
        for index, row in particles.iterrows():
            ...
            if ctf:
                ...
                if not ctf_volume.shape == dim:
                    subtomo = np.fft.irfftn(np.fft.rfftn(subtomo_in)*ctf_volume,
                                            s=subtomo_in.shape)
    ...
    starfile.write({'optics': star_optics, 'particles': particles_2d},
...
def apply_subset(subset_star, subtomo_stars):
    ...
    Give the star file from subset selection job in relion and the (multiple) star files
    in which subtomograms are listed. Will output one starfile for each subtomogram star,
    in the structure expected for Warp. Uses the filename of the projections stack to match.
    ...
    for st_star in subtomo_stars:
        ...
        starfile.write({'optics': fullset_3d['optics'], 'particles': subset_3d}, 
